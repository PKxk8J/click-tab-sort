...
タイトル順は Firefox の UI 言語に合わせます。日本語では、記号、数字、ラテン文字、かな、漢字、
その他の文字体系の順に一文字ずつ比較します。大文字・タイトルケース・小文字、全角半角、
平仮名片仮名、数字体系、表示専用互換形の違いは主な比較で無視します。文字列として同値だったときだけ、
半角を全角より先に、平仮名を片仮名より先にするなど、元の表記を区別します。
数字も一文字ずつ比較するため、`1`、`10`、`2` の順になります。
