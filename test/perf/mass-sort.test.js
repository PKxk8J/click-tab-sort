...
describe('Firefox real tab sorting performance', () => {
  ...
  test('大量の実タブをパフォーマンス予算内にURL順でソートする', {
    ...
    const result = await runExtensionScript(`
      ...
      try {
        testWindow = await browser.windows.create({
          focused: true,
          url: makeUrl(descendingIndexes[0]),
        })
        ...
        for (const index of descendingIndexes.slice(1)) {
        ...
        const readyTabs = await waitUntil(async () => {
          ...
          return tabs.every((tab, index) =>
            tab.url === makeUrl(descendingIndexes[index])) && tabs
        }, 120000)
        if (!readyTabs) {
        ...
        await run(testWindow.id, 'url', false, false, 'currentArea',
          createdTabs[0].id)
        ...
        if (!orderedTabs) {
          ...
          const orderedUrls = tabs.
            ...
            map(tab => tab.url)
          throw new Error(
            'tabs were not sorted by URL. expected ' +
    ...
    assert.ok(
      ...
      `expected ${tabCount} real tabs to be sorted within ${maxMs}ms, ` +
